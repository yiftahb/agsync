...
function managedHeader(sourcePath: string): string {
  return [
...
function buildOutputSkillMd(skill: ResolvedSkill, tools: ToolDefinition[]): string {
  ...
  lines.push(managedHeader(skillSourcePath(skill)));
  lines.push("---");
...
function buildAgentsMd(
  ...
  lines.push(managedHeader(sourcePath));
...
function buildOutputCommand(cmd: CommandDefinition): string {
  const header = managedHeader(commandSourcePath(cmd));
  if (cmd.scope) {
    return header + cmd.content +
