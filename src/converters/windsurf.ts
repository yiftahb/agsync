...
export class WindsurfConverter extends BaseAgentConverter {
  ...
  convert(config: ResolvedConfig, outputDir: string): ConvertedOutput {
    ...
    if (mcpTools.length > 0) {
      ...
      files.push({
        path: resolve(outputDir, ".windsurf", "mcp_config.json"),
  ...
  async detect(): Promise<boolean> {
    try {
      await execFileAsync("windsurf", ["--version"]);
      return true;
    } catch {
      return false;
    }
  ...
  getOutputPaths(outputDir: string): string[] {
    return [
      resolve(outputDir, ".windsurf", "mcp_config.json"),
    ];
  }
}
